...
void main(){
	...
	if(d < 0.4){
		float c = dot(cross(normalized(phi_u(pos_normalized, 0.4)), normalized(phi_v(pos_normalized, 0.4))), normalized(normalized_mouse_pos -(phi(pos_normalized, 0.4) - vec3(0.0, 0.0, 100.0*light_d*light_d))));
